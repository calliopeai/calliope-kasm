# Chat Studio for Kasm Workspaces

Browser-accessible AI-powered data analysis and SQL agent environment powered by [Kasm Workspaces](https://kasmweb.com).
...
```bash
docker run --rm -it --shm-size=512m -p 6901:6901 -e VNC_PW=password calliopeai/calliope-chat-studio-kasm:latest
```
...
2. Docker Image: `calliopeai/calliope-chat-studio-kasm:latest`
...
- **Interactive Charts** - Visualize query results with built-in charting
...
Chat Studio is free to use. See [calliope.ai](https://calliope.ai) for terms.
